...
namespace WebservicesSage
{
    public partial class MainUI : Form
    {
        ...
        public MainUI()
        {
            ...
            ZohoController test = new ZohoController("test");
            InitializeComponent();
            InitCustomLabelFont();
            ...
            SingletonUI.Instance.MenuTitle = MenuTitle; //MenuTitle;
            ...
            SingletonUI.Instance.CategorieCompta = CategorieCompta;
        ...
        private void SyncClient_Click(object sender, EventArgs e)
        {
            ...
            if (!promptValue.Equals("CLOSE"))
            {
                ...
                {
                    ControllerClient.SendProspectCron();
        ...
        {
            string promptValue = ShowDialog("Laisser vide pour synchroniser tout les Prospects", "CT_num Prospect");
            if (!promptValue.Equals("CLOSE"))
            {
                if (!String.IsNullOrEmpty(promptValue))
                {
                    ControllerClient.SendProspect(promptValue);
                }
                else
                {
                    SingletonServices.Instance.ServiceClient.sendProspect();
                }
            }

        }
        private void SynchroProducts_Click(object sender, EventArgs e)
        {
            
            
            try
            {
                //ControllerClient.SendClientCron();
                string promptValue = ShowDialog("Laisser vide pour synchroniser tout les documents", "CT_num client");
                if (!promptValue.Equals("CLOSE"))
                {
                    if (!String.IsNullOrEmpty(promptValue))
                    {
                        SingletonServices.Instance.ServiceClient.SendSaleDocument(promptValue);
                    }
                    else
                    {
                        SingletonServices.Instance.ServiceClient.SendSaleDocument();
                    }
                }
                MessageBox.Show("end supp doc", "ok",
            ...
            //ControllerArticle.SendZohoArticle();
        }
        private void SynchroSalesOrders_Click(object sender, EventArgs e)
        {

        }

        private void CustomImageButto1_Click(object sender, EventArgs e)
        {
